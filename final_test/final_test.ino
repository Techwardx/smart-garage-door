...
const int activeStartHour = 19; // 7 PM
const int alertStartHour = 21;  // 9 PM
const int alertEndHour = 24;    // Midnight

// Deep sleep duration
const uint64_t sleepDuration = 300000000; // 5 mintues in microseconds
...
void setup() {
  ...
  bot.sendMessage(CHAT_ID, "Current state: " + String(sensorState ? "✅ OPEN" : "❌ CLOSED"), "");
...
void loop() {
  ...
  if (currentTime - lastCheckTime >= checkInterval) {
    ...
    if (sensorState != lastSensorState) {
      ...
      message += "New State: ";
      message += sensorState ? "✅ OPEN" : "❌ CLOSED";
      message += "\nTime: ";
    ...
    if (isInAlertPeriod() && sensorState == true) {
...
// Check if in alert period (9PM-midnight)
...
void goToSleep() {
  ...
  if (getLocalTime(&timeinfo)) {
    ...
    bot.sendMessage(CHAT_ID, "😴 Going to sleep for 5 minutes...", "");
  }
  ...
  for (int i = 0; i < numNewMessages; i++) {
    ...
    if (chat_id == CHAT_ID) {
      ...
      if (text == "status" || text == "/status" || text == "Status") {
        ...
        response += "State: ";
        response += sensorState ? "✅ OPEN" : "❌ CLOSED";
        response += "\nTime: ";
